...
{
    class Randoms : Processor
    {
        public static TextBlock random(List<double> process, int N, int maxThr, int minThr, int retries, int max_time)
        {
            ...
            for (int i = 0; i < max_time; i++)
            {
                foreach (Processor p in processor)
                {
                    ...
                    {
                        int retries_temp = retries;
                        while (retries_temp != 0)
                        {
                            int rand_processor = rand.Next(0, N);
                            sum_of_request += 1;
                            if (processor[rand_processor].calc_util() < maxThr)
                            {
            ...
            Console.WriteLine("Random processor request, average utilization of each processor:");
            for (int temp = 0; temp < processor.Count(); temp++)
                sum_of_means += processor[temp].get_mean_util();

            block.Text += "   Random processor request, standard deviation of each processor: " + Environment.NewLine;
